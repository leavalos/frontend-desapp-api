        ...
        <div class="md-layout">
          ...
          >
            <login-card header-color="green">
              <h4 slot="title" class="card-title">{{registered? $t('loggin.register')  : $t('loggin.loggin')}}</h4>
              <md-button
                slot="buttons"
              ...
              <v-facebook-login ref="fbcomponent" v-show="false"
              v-if="registered" 
              ...
              <md-field  class="md-form-group" slot="inputs" :class="{'md-valid': !error, 'md-valid md-invalid': !!error}" v-show="registered" >
                <md-icon>face</md-icon>
                <label for="nickname" ><span style="color: black;">{{ $t('loggin.nickname') }}</span></label>
                <md-input name="nickname" v-model="nickname" :required="true" @change="validNickname" ></md-input>
                <span class="md-error">{{ $t('landing.modal.error') }}</span>
              </md-field>


              <md-field class="md-form-group" slot="inputs" :class="{'md-valid': !errorEmail, 'md-valid md-invalid': !!errorEmail}"  style="padding-top: 15px;">
                <md-icon>email</md-icon>
                <label for="email" ><span style="color: black;">{{ $t('loggin.email') }}</span></label>
                <md-input name="email" v-model="email" type="email" :required="true" @change="validEmail"></md-input>
                <span class="md-error">{{ $t('landing.modal.error') }}</span>
              </md-field>
              ...
              <md-button v-if="(errorEmail || errorPass) || (!password && !email)" slot="footer" class="md-simple md-success md-lg" @click="loggin" disabled v-show="!registered">
                {{ $t('loggin.loggin') }}
              </md-button>
              <md-button v-else slot="footer" class="md-simple md-success md-lg" @click="loggin" v-show="!registered">
                {{ $t('loggin.loggin') }}
              </md-button>
